...
function findingBase(
  ...
  return { id, title, severity, affectedPages, estimatedFixTime: fixTime, estimatedImpact: impact, whatIsIt, whyItMatters, howToFix };
}
...
const checkFAQSchema: RuleCheck = (_homepage, allPages) => {
  for (const page of allPages) {
    ...
    return findingBase(
      ...
      30,
      8,
      "FAQPage schema is invisible code that marks up your Q&A content so AI systems can read it as structured questions and answers. You already have this set up, but it needs more questions. AI looks for at least 5–10 solid pairs to treat your page as a real FAQ source.",
  ...
  return findingBase(
    ...
    60,
    8,
    "FAQPage schema is invisible code that marks up your Q&A content so AI systems can read it as a structured list of questions and answers. Without it, AI has to guess which text on your page is a question and which is an answer — and often gets it wrong or ignores it entirely.",
...
const checkContentFreshness: RuleCheck = (homepage) => {
  ...
  if (ageMs > days180) {
    return findingBase(
      ...
      15,
      7,
      "Content freshness is a signal AI systems use to decide if your information is trustworthy and current. It's based on the Last-Modified date of your page — a hidden timestamp that tells AI when you last made changes. Pages that haven't been updated in months get deprioritized because AI assumes your hours, prices, or team may have changed.",
  ...
  if (ageMs > days90) {
    return findingBase(
      ...
      15,
      7,
      "Content freshness is a signal AI systems use to decide if your information is trustworthy and current. It's based on the Last-Modified date of your page — a hidden timestamp that tells AI when you last made changes. Pages that haven't been touched in 90+ days start to lose citation priority.",
...
const checkMetaDescription: RuleCheck = (homepage) => {
  ...
  const metaWhatIsIt = "The meta description is a 1–2 sentence summary of your page that lives in the <head> of your website's code — invisible to visitors but read by search engines and AI. It's the grey description text that appears under your business name in Google search results, and AI systems use it to quickly understand what your page is about.";
...
const checkCredentials: RuleCheck = (homepage) => {
  ...
  const credWhatIsIt = "E-E-A-T stands for Experience, Expertise, Authoritativeness, and Trustworthiness — the criteria AI systems and Google use to decide which businesses to recommend. Credentials signals are the visible proof points on your website that show you're legitimate: years in business, licenses, awards, customer reviews, and media mentions. The more of these you display, the more trustworthy your site looks to AI.";
...
const checkCitationConsistency: RuleCheck = () => {
  return findingBase(
    ...
    90,
    8,
    "A citation is any online mention of your business name, address, and phone number — called NAP data. Citations appear on Google Business Profile, Yelp, BBB, and hundreds of directory sites. AI systems cross-reference these sources to verify your business information. If your address says \"123 Main St\" on your website but \"123 Main Street\" on Yelp, AI treats that as a discrepancy and reduces its confidence in recommending you.",
...
const checkTitleTag: RuleCheck = (homepage) => {
  ...
  const titleWhatIsIt = "The title tag is the text that appears in the browser tab at the top of your screen. It's also the blue clickable headline shown in Google search results. It lives in the <head> of your website's code and is one of the first things AI systems and search engines read to understand what your business does and where you're located. Format: \"[Service] in [City] | [Business Name]\".";
